...
import copy
from import_mnist import imload, imloadq
...
te_vec, te_im, te_id, te_ohid1 = imloadq('test', 60000, 28, 28, 28, 28, 0)
    ...
    for i in range(12001):
        ...
        if i % 1000 == 0 and i >= 1000:
            ...
            print(str(i) + ": ********* epoch " + str(
                i * 100 // 60000 + 1) + " ********* test accuracy:" + str(
...
def CNN_KF18do_RBF_AUG():
    ...
    # Y5 = tf.matmul(YY1, W15) + tf.matmul(YY2, W25) + tf.matmul(YY3, W35) + tf.matmul(YY, W5) + B5
    # Y7 = tf.matmul(YY, W7) + B7
    Y37 = tf.reduce_mean(tf.reduce_mean(Y3, axis=1),
                         axis=1)  # tf.matmul(tf.reshape(Y3, shape=[-1, 144 * J]), W37) + B37
    ...
    for i in range(12001):
        ...
        sess.run(update_ema,
                 {X0: batch_X, Y_: batch_Y, step: i, pkeep: 1.0, pkeep2: 1.0, tst: False, ba_size: bs})
        sess.run(train_step,
                 {X0: batch_X, Y_: batch_Y, step: i, pkeep: 0.7, pkeep2: 0.7, tst: False, ba_size: bs})
...
def CNN_KF18do24_RBF_AUG():
    ...
    for i in range(12001):
        ...
        sess.run(update_ema,
                 {X0: batch_X, Y_: batch_Y, step: i, pkeep: 1.0, pkeep2: 1.0, tst: False, ba_size: bs})
        sess.run(train_step,
                 {X0: batch_X, Y_: batch_Y, step: i, pkeep: 0.6, pkeep2: 0.8, tst: False, ba_size: bs})
...
def CNN_KF18kf1_RBF_AUG():
    ...
    lr = 0.0000001 + tf.train.exponential_decay(0.001, step, 1000, 1 / math.e)  # cut lr in future
    train_step = tf.train.AdamOptimizer(lr).minimize(0.0 * class_error0 + 1.0 * class_error + cross_entropy)
    #train_step = tf.train.AdamOptimizer(lr).minimize(cross_entropy)
    #train_step = tf.train.MomentumOptimizer(10 * lr, 0.9).minimize(0.1 * class_error + cross_entropy)
    train_step1 = tf.train.AdamOptimizer(lr).minimize(class_error, var_list=[W1, B1, W2, B2, W3, B3, W4, B4])
    train_step2 = tf.train.AdamOptimizer(lr).minimize(mse, var_list=[W5, B5, W6, B6, W7, B7])
